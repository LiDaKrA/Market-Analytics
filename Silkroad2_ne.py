...
# ---------------------------------------------------------------------
#---- tu- Incorporate review-update structure of the crawls
#---- tu- category_id in Reviews table
#---- tu- Make sure collections are homogenous
#---- tu- Updating-Values Functions
#---- tu- Updating profiles - figure out how (Hashes?)
# ---------------------------------------------------------------------
...
def CollectVendorInformation(userdir, fdate):
    ...
    if vendor != 'gwern':
        ...
        keys = session.query(PGP_Keys).filter(and_(PGP_Keys.marketplace_id=='SR2', PGP_Keys.vendor_id==vendor_id)).all()
        if keys:
            isThere = False
            for key in keys:
        ...
        profiles = session.query(Profiles).filter(and_(Profiles.marketplace_id=='SR2', Profiles.vendor_id==vendor_id)).all()  
        if profiles:       
            isThere = False
    ...
    try:
        ...
        with open(pagelist[0], encoding='utf8') as fname:
            ...
            if soup.find_all(id = 'content') and soup.find_all('h2'):  
                ...
                #------ Review Class
                # AAA update reviews since last reviewdate in the database
                # This will depend on newProduct s status
                # Loop over pages again
                        ...
                        if soup.find_all(id = 'content') and soup.find_all('h2'):  
                            ...
                            for i in range(1, int(reviews)+1): 
                                ...
                                if prices!=None:
                                    ...
                                    reviewPrice = session.query(Prices).filter(and_(Prices.marketplace_id=='SR2', Prices.product_id==product_id, Prices.vendor_id==vendor_id)).order_by(asc(Prices.timestamp)).first().price
...
def main():
    ...
    for datestr in crawls[4:]: #['2014-04-12']
